...
class ConversationGraphPipeline:
    ...
    """
    def __init__(self, raw_train_path, raw_dev_path, raw_test_path, inspired_movie_map_path, 
                 movielens_links_path, mappings_path):
        
        # This dictionary correctly stores the paths for each data split
        self.paths = { 
    ...
    def _clean_text(self, text):
        ...
        if not isinstance(text, str):
            return None
        text = re.sub(r"[^a-zA-Z0-9\s.,?'!-]", '', text) # Your correct regex
        return text if text else None

    
    def generate_and_build_data(self, mode, model_name='all-MiniLM-L6-v2', output_pt_path_prefix=None):
        """
        Loads all data for a specific split (train, dev, or test),
        finds valid mappable conversations, aggregates text, creates
        embeddings, and builds the conversation_x, edge_index, and edge_attr.
        """        
        print(f"--- Running Pipeline for {mode.upper()} SPLIT ---")
        
        print("Loading all data and map files...")
        
        # Load the correct split file using the 'mode'
        df_split = pd.read_csv(self.paths[mode], sep='\t', on_bad_lines='skip')
        ...
        print(f"Pre-scanning {mode.upper()} conversations to find valid, mappable edges...")
        ...
        df_split = df_split.dropna(subset=['clean_text', 'movie_id', 'fine_label'])
        
        agg_text_map = df_split.groupby('dialog_id')['clean_text'].apply(' '.join).to_dict()
        conv_movie_map = df_split.groupby('dialog_id')['movie_id'].last().to_dict()
        conv_outcome_map = df_split.groupby('dialog_id')['fine_label'].last().to_dict()
        ...
        # Create local variables for the final tensors
        ...
        print(f"\nFinal {mode.upper()} conversation feature matrix shape: {conversation_x.shape}")
        print(f"Final {mode.upper()} new edge_index shape: {conv_movie_edge_index.shape}")
        print(f"Final {mode.upper()} new edge_attr shape: {conv_movie_edge_attr.shape}")
