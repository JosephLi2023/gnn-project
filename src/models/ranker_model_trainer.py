...
class QueryCandidatesDataset(Dataset):
    """
    Each item returns:
      - query_emb: torch.Tensor [D_q]
      - candidates: Dict[int, torch.Tensor[D_deep]]
      - label_movie_id: int
    """
    def __init__(self, queries: List[torch.Tensor], candidates: List[Dict[int, torch.Tensor]], labels: List[int]):
        assert len(queries) == len(candidates) == len(labels), "Dataset lengths must match"
...
if __name__ == "__main__":
    ...
    ds = QueryCandidatesDataset(queries, candidates, labels)
    dl = DataLoader(ds, batch_size=8, shuffle=True, num_workers=0, collate_fn=collate_identity)
